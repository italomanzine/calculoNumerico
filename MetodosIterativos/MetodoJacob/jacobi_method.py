...
def jacobi_method(A, B, tolerance, max_iterations):
    """
    Executa o Método Iterativo de Jacobi para resolver o sistema de equações.
    ...
    new_X = np.zeros_like(B)  # Vetor para armazenar novas estimativas
...
def main():
    ...
    while True:
        ...
        print("3. Executar o Método Iterativo de Jacobi")
        ...
        if choice == '1':
            ...
            if A is None or B is None or tolerance is None or max_iterations is None:
                ...
                X, iterations, iterations_list = jacobi_method(A, B, tolerance, max_iterations)
                ...
                for idx, sol in enumerate(iterations_list):
                    print(f"Iteração {idx + 1}: X = {np.round(sol, 4)}")
                print("Soluções encontradas:", np.round(X, 4))
