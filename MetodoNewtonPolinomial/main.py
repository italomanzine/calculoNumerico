...
def main():
    ...
    # Solicita o intervalo do usuário
    x_start = float(input("Digite o valor inicial do intervalo: "))
    x_end = float(input("Digite o valor final do intervalo: "))
    ...
    root, iterations_data = find_roots(f, df, x_start, x_end, tol, max_iter)  # Encontra as raízes

    if root is not None:
        print(f"A raiz aproximada é: {root}.")
        print_table(iterations_data)  # Exibe a tabela de iterações
        plot_roots(f, root, x_start, x_end)  # Exibe o gráfico
    else:
